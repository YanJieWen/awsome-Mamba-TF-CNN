'''
@File: bi_selective_scan_interface.py
@Software: PyCharm
...
from .ssm_methods import selective_scan_fn
...
def bimamba_inner_ref(
    ...
    bimamba-version1
    :param xz: b,d,l
    :param conv1d_weight: fan_out,fan_in,k
    :param conv1d_bias: fan_out
    :param x_proj_weight: d_rank+2*d_s,d_in-->将编码的x映射为dt,B,C
    :param delta_proj_weight:d_in,d_rank-->离散化张量
    :param out_proj_weight: d,d_in-->转换层
    :param out_proj_bias:d_in
    :param A:[d_in,d_s] 演化参数
    :param A_b:[d_in,d_s] 反向演化参数
    :param B: [b,d_s,l] 映射参数
    :param C: [b,d_s,l] 映射参数
    :param D: [d_in]阶跃参数
    :param delta_bias: d_in
    :param B_proj_bias: None
    :param C_proj_bias: None
    ...
    delta = delta_proj_weight@x_dbl[:,:delta_rank].t()#[d_in,bl]
    delta = rearrange(delta,'d (bl)->b d l',l=L)#b,d_in,l
    if B is None:
        B = x_dbl[:, delta_rank:delta_rank + d_state]
        if B_proj_bias is not None:
            B = B + B_proj_bias.to(dtype=B.dtype)
        B = rearrange(B, "(b l) dstate -> b dstate l", l=L).contiguous()#b,d_s,l
    if C is None:
        C = x_dbl[:,-d_state:]
        if C_proj_bias is not None:
            C = C+C_proj_bias.to(dtype=C.dtype)
        C = rearrange(C,'(bl) dstate->b dstate l',l=L).contiguous()#b,d_s,l
        #softplus保证正数
    y = selective_scan_fn(x,delta,A,B,C,D,z=z,delta_bias=delta_bias,delta_softplus=delta_softplus)
    y_b = selective_scan_fn(x.flip([-1]),delta.flip([-1]),A_b,B.flip([-1]),C.flip([-1]),D,z=z.flip([-1]),delta_bias=delta_bias,delta_softplus=delta_softplus)
    y = y + y_b.flip([-1])
    return F.linear(rearrange(y, "b d l -> b l d"), out_proj_weight, out_proj_bias)

def bimamba_inner_fn(xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
    out_proj_weight, out_proj_bias,
    A, A_b, B = None, C = None, D = None, delta_bias = None, B_proj_bias = None,
    C_proj_bias = None, delta_softplus = True):
...
def mamba_inner_ref_no_out_proj(xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
    A, B=None, C=None, D=None, delta_bias=None, B_proj_bias=None,
    C_proj_bias=None, delta_softplus=True):
    L = xz.shape[-1]
    delta_rank = delta_proj_weight.shape[1]
    ...
    x_dbl = F.linear(rearrange(x,'b d l->(b l) d'),x_proj_weight)
    delta = delta_proj_weight@x_dbl[:,:delta_rank].t()#[d_in,bl]
    delta = rearrange(delta,'d (b l)->b d l',l=L)#b,d_in,l
    if B is None:
        B = x_dbl[:, delta_rank:delta_rank + d_state]
        if B_proj_bias is not None:
            B = B + B_proj_bias.to(dtype=B.dtype)
    ...
    if C is None:
        ...
        if C_proj_bias is not None:
            C = C+C_proj_bias.to(dtype=C.dtype)
        C = rearrange(C,'(b l) dstate->b dstate l',l=L).contiguous()#b,d_s,l
        #softplus保证正数
    y = selective_scan_fn(x,delta,A,B,C,D,z=z,delta_bias=delta_bias,delta_softplus=delta_softplus)
...
def mamba_inner_fn_no_out_proj(xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
    ...
    # return mamba_inner_ref_no_out_proj(xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
